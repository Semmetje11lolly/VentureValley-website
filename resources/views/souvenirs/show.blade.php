<x-default-layout title="{{ $souvenir->name }} • VentureValley">
    ...
    <x-slot name="header">
        <x-header height="large" image="{{ asset('storage/' . $souvenir->background_image) }}">
            @if(!$souvenir->public)
            ...
            @forelse($souvenirs as $souvenir)
                <a href="{{ route('souvenirs.show', $souvenir->slug) }}">
                    <article
                        ...
                        style="background-image: linear-gradient(180deg, #00000000 70%, #000000 100%), url({{ asset('storage/' . $souvenir->list_image) }}); box-shadow: 0 0 10px 0 rgba(0, 0, 0, 0.5); corner-shape: scoop">
